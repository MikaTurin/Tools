...
class Collection implements ArrayAccess, Countable, IteratorAggregate
{
    ...
    public function contains($key, $value = null)
    {
        if (func_num_args() == 2) {
            return $this->contains(function ($k, $item) use ($key, $value) {
                return \App\Collection::data_get($item, $key) == $value;
            });
        }
     ...
     * @param \App\Collection | \App\ArrayableInterface | array $items
     * @return static
     */
    ...
    {
        return new static(static::array_fetch($this->items, $key));
    }
    ...
    public function where($key, $value, $strict = true)
    {
        return $this->filter(function ($item) use ($key, $value, $strict) {
            return $strict ? \App\Collection::data_get($item, $key) === $value
                : \App\Collection::data_get($item, $key) == $value;
        });
    }
    ...
    {
        return new static(static::array_flatten($this->items));
    }
    ...
    protected function getGroupByKey($groupBy, $key, $value)
    {
        ...
        return static::data_get($value, $groupBy);
    }
    ...
    /**
     * Set values in array by a field or using a closure.
     * @param  string
     * @return static
     */
    public function valueBy($valueBy)
    {
        return $this->transform($this->valueRetriever($valueBy));
    }
    ...
    {
        return static::array_pull($this->items, $key, $default);
    }
    ...
    public function valueRetriever($value)
    {
        return function ($item) use ($value) {
            return \App\Collection::data_get($item, $value);
        };
    }
     ...
     * @return array
     */
    protected static function array_fetch($array, $key)
    {
        $results = array();
     ...
     * @return mixed
     */
    protected static function array_first($array, $callback, $default = null)
    {
     ...
     * @return array
     */
    protected static function array_pluck($array, $value, $key = null)
    {
        $results = array();
     ...
     * @return void
     */
    protected static function array_forget(&$array, $keys)
    {
        $original =& $array;
